...
bnda_cty_path = cwd / "../data/BNDA_CTY.shp"
urls_path = cwd / "../hdx/urls"
...
def get_iso3() -> list[str]:
    ...
    iso3 = read_file(bnda_cty_path, columns=["iso3cd"])
    return iso3["iso3cd"].tolist()


def get_years() -> list[int]:
    """Get list of years contained in the STAC file."""
    years = read_parquet(stac_path, columns=["datetime"])
        ...
        for year in years:
            with duckdb.connect() as con:
                ...
                query = f"""
                    ...
                    ON ST_Intersects(stac.geometry, bnda_cty.geom)
